...
1) check "NetworkLib" on "Project References" page
...
2) Modify "YourApplication.pro" file in your application to contain
  ...
  2a) when using shared library:
      ...
      include($$quote($$_PRO_FILE_PWD_)/../NetworkLib/shared.pri)
...
3) In case you are using shared library, modify "bar-descriptor.xml" file
  ...
  3a) in <configuration name="Device-Debug"> tag:
       <asset path="../NetworkLib/arm/so.le-v7-g/" entry="false" dest="lib"><include name="libNetworkLib.so.*"/></asset>

  3b) in <configuration name="Device-Release"> tag:

       <asset path="../NetworkLib/arm/so.le-v7/" entry="false" dest="lib"><include name="libNetworkLib.so.*"/></asset>

  3c) in <configuration name="Device-Profile"> tag:

       <asset path="../NetworkLib/arm-p/so.le-v7-g/" entry="false" dest="lib"><include name="libNetworkLib.so.*"/></asset>

  3d) in <configuration name="Simulator-Debug"> tag:

       <asset path="../NetworkLib/x86/so-g/" entry="false" dest="lib"><include name="libNetworkLib.so.*"/></asset>
...
static one, add the following line to "NetworkLib.pro" file:
...
1) Via set solib-search-path GDB command:
  ...
  1d) In the log output, write the following line to add
    ...
    1da) For BB10 devices:
         ...
         set solib-search-path /ABSOLUTE/PATH/TO/WORKSPACE/NetworkLib/arm/so.le-v7-g/

    1db) For BB10 simulators:

         set solib-search-path /ABSOLUTE/PATH/TO/WORKSPACE/NetworkLib/x86/so-g/
...
2) Via Debugger Launch Configuration dialog:
  ...
  2d) Use "Add..." button to specify path to your:
    ...
    2da) For BB10 devices, NetworkLib/arm/so.le-v7-g/ folder in workspace

    2db) For BB10 simulators, NetworkLib/x86/so-g/ folder in workspace

How to generate separate debug info
===================================

By default, the shared library contain debug info. If you want to build
a separate debug info file, add the following line to "NetworkLib.pro" file:
...
1) When using .qm files directly:
    ...
    1a) Add the following code in the "bar-descriptor.xml" file in your application
        ...
        <asset path="../NetworkLib/translations/qm" dest="qm">
            <include name="*.qm"/>
        </asset>
    ...
    1d) Add the following code to "ApplicationUI::onSystemLanguageChanged"
        ...
        file_name = QString("NetworkLib_%1").arg(locale_string);
...
2) When using qm files via QRC file:
    ...
    2a) Uncomment the following line in "NetworkLib.pro" file:
    ...
    2d) Add the following code to "ApplicationUI::onSystemLanguageChanged"
        ...
        QCoreApplication::instance()->removeTranslator(m_pLibraryTranslator);
        file_name = QString(":/NetworkLib_%1.qm").arg(locale_string);
    ...
    2e) When using static library, you need to add the following line
        ...
        Q_INIT_RESOURCES(NetworkLib_translations)
